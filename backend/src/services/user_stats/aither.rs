...
pub struct AitherScraper;

#[derive(Debug, Deserialize)]
struct AitherResponse {
    group: String,
    uploaded: String,
    downloaded: String,
    ratio: String,
...
impl From<AitherResponse> for UserProfileScraped {
    fn from(wrapper: AitherResponse) -> Self {
        UserProfileScraped {
...
impl Scraper for AitherScraper {
    async fn scrape(&self, indexer: Indexer) -> Result<UserProfileScraped> {
        ...
        let response = serde_json::from_str::<AitherResponse>(&body).map_err(|e| {
