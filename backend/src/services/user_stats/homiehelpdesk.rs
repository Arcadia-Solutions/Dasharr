...
pub struct HomieHelpDeskScraper;

#[async_trait]
impl Scraper for HomieHelpDeskScraper {
    async fn scrape(
        &self,
        indexer: Indexer,
        ...
        let res = client
            .get(format!(
                "https://homiehelpdesk.net/api/user?api_token={}",
                indexer
                    .auth_data
                    .get("api_key")
                    .ok_or("HomieHelpDesk API key not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .get("value")
                    .ok_or("HomieHelpDesk API key value not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .as_str()
                    .unwrap()
            ))
