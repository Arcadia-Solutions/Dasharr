...
pub struct RastastuganScraper;

#[async_trait]
impl Scraper for RastastuganScraper {
    async fn scrape(
        &self,
        indexer: Indexer,
        ...
        let res = client
            .get(format!(
                "https://rastastugan.org/api/user?api_token={}",
                indexer
                    .auth_data
                    .get("api_key")
                    .ok_or("Rastastugan API key not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .get("value")
                    .ok_or("Rastastugan API key value not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .as_str()
                    .unwrap()
            ))
