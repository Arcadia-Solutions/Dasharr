...
pub struct Racing4EveryoneScraper;

#[async_trait]
impl Scraper for Racing4EveryoneScraper {
    async fn scrape(
        &self,
        indexer: Indexer,
        ...
        let res = client
            .get(format!(
                "https://racing4everyone.eu/api/user?api_token={}",
                indexer
                    .auth_data
                    .get("api_key")
                    .ok_or("Racing4Everyone API key not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .get("value")
                    .ok_or("Racing4Everyone API key value not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .as_str()
                    .unwrap()
            ))
