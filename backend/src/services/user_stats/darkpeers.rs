...
pub struct DarkPeersScraper;

#[async_trait]
impl Scraper for DarkPeersScraper {
    async fn scrape(
        &self,
        indexer: Indexer,
        ...
        let res = client
            .get(format!(
                "https://darkpeers.org/api/user?api_token={}",
                indexer
                    .auth_data
                    .get("api_key")
                    .ok_or("DarkPeers API key not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .get("value")
                    .ok_or("DarkPeers API key value not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .as_str()
                    .unwrap()
            ))
