...
pub struct ItaTorrentsScraper;

#[async_trait]
impl Scraper for ItaTorrentsScraper {
    async fn scrape(&self, indexer: Indexer) -> Result<UserProfileScraped> {
        let client = reqwest::Client::new();
        let res = client
            .get(format!(
                "https://itatorrents.xyz/api/user?api_token={}",
                indexer
                    .auth_data
                    .get("api_key")
                    .ok_or("ItaTorrents API key not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .get("value")
                    .ok_or("ItaTorrents API key value not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .as_str()
                    .unwrap()
            ))
