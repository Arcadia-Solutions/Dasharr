...
pub struct BlutopiaScraper;

#[derive(Debug, Deserialize)]
struct BlutopiaResponse {
    group: String,
    uploaded: String,
    downloaded: String,
    ratio: String,
...
impl From<BlutopiaResponse> for UserProfileScraped {
    fn from(wrapper: BlutopiaResponse) -> Self {
        UserProfileScraped {
...
#[async_trait]
impl Scraper for BlutopiaScraper {
    async fn scrape(&self, indexer: Indexer) -> Result<UserProfileScraped> {
        let client = reqwest::Client::new();
        let res = client
            .get(format!(
                "https://blutopia.cc/api/user?api_token={}",
                indexer
                    .auth_data
                    .get("api_key")
                    .ok_or("BLU API key not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .get("value")
                    .ok_or("BLU API key value not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .as_str()
                    .unwrap()
            ))
        ...
        let response = serde_json::from_str::<BlutopiaResponse>(&body).map_err(|e| {
