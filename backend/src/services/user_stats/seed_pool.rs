...
pub struct SeedPoolScraper;

#[async_trait]
impl Scraper for SeedPoolScraper {
    async fn scrape(&self, indexer: Indexer) -> Result<UserProfileScraped> {
        let client = reqwest::Client::new();
        let res = client
            .get(format!(
                "https://seedpool.org/api/user?api_token={}",
                indexer
                    .auth_data
                    .get("api_key")
                    .ok_or("SeedPool API key not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .get("value")
                    .ok_or("SeedPool API key value not found")
                    .map_err(|e| Error::CouldNotScrapeIndexer(e.into()))?
                    .as_str()
                    .unwrap()
            ))
